...
const { status, data } = await apiInstance.apiAssetTypesGet(
    page,
    pageSize
);
```
...
const { status, data } = await apiInstance.apiAssetTypesIdDelete(
    id
);
```
...
const { status, data } = await apiInstance.apiAssetTypesIdGet(
    id
);
```
